...
class ColecaoRestaurantes {
    ...
    public Restaurante buscarPorNome(String nome) {
        int i = 0; while (i < tamanho) { if (Texto.igual(restaurantes[i].getNome(), nome)) return restaurantes[i]; i++; } return null;
    }
}

/* ---- Nivel 2: AVL por nome (minusculo) ---- */
class NoAVL {
    Restaurante restaurante; NoAVL esq, dir; int altura;
    NoAVL(Restaurante r) { restaurante = r; esq = null; dir = null; altura = 1; }
}

class AVL {
    NoAVL raiz;
    static long comparacoes = 0;
    private int h(NoAVL n) { return n == null ? 0 : n.altura; }
    private int max(int a, int b) { return a > b ? a : b; }
    private NoAVL rotDir(NoAVL y) { NoAVL x = y.esq; NoAVL t = x.dir; x.dir = y; y.esq = t; y.altura = 1+max(h(y.esq),h(y.dir)); x.altura = 1+max(h(x.esq),h(x.dir)); return x; }
    private NoAVL rotEsq(NoAVL x) { NoAVL y = x.dir; NoAVL t = y.esq; y.esq = x; x.dir = t; x.altura = 1+max(h(x.esq),h(x.dir)); y.altura = 1+max(h(y.esq),h(y.dir)); return y; }
    public void inserir(Restaurante r) { raiz = ins(raiz, r); }
    private NoAVL ins(NoAVL n, Restaurante r) {
        if (n == null) return new NoAVL(r);
    ...
    public String pesquisar(String nome, String prefixo) { return pesquisar(raiz, nome, prefixo + "raiz"); }
    private String pesquisar(NoAVL n, String nome, String caminho) {
        String resp = caminho + " NAO";
        if (n != null) {
            comparacoes++;
            int c = nome.compareTo(n.restaurante.getNome());
            if (c == 0) { resp = caminho + " SIM " + n.restaurante.formatar(); }
    ...
    private void emOrdem(NoAVL n) { if (n != null) { emOrdem(n.esq); System.out.println(n.restaurante.formatar()); emOrdem(n.dir); } }
}

/* ---- Nivel 1: ABP por (capacidade % 15) (maiusculo) ---- */
class NoBST {
    int chave; AVL avl; NoBST esq, dir;
    NoBST(int chave) { this.chave = chave; avl = new AVL(); esq = null; dir = null; }
}

class ArvoreDupla {
    private NoBST raiz;
    static long comparacoesBST = 0;

    public void inserir(Restaurante r) { raiz = inserir(raiz, r); }
    private NoBST inserir(NoBST n, Restaurante r) {
        int chave = r.getCapacidade() % 15;
        if (n == null) { NoBST novo = new NoBST(chave); novo.avl.inserir(r); return novo; }
        if (chave < n.chave) { n.esq = inserir(n.esq, r); }
        else if (chave > n.chave) { n.dir = inserir(n.dir, r); }
        else { n.avl.inserir(r); }
        return n;
    }

    public void pesquisar(String nome, int capacidade) {
        System.out.println(pesquisarRec(raiz, nome, "RAIZ"));
    }
    private String pesquisarRec(NoBST n, String nome, String caminho) {
        if (n == null) return caminho + " NAO";
        comparacoesBST++;
        String av = n.avl.pesquisar(nome, caminho + " ");
        boolean nao = av.charAt(av.length()-3) == 'N' && av.charAt(av.length()-2) == 'A' && av.charAt(av.length()-1) == 'O';
        if (!nao) return av;
        String path = Texto.faixa(av, 0, av.length() - 4);
        String esq = pesquisarRec(n.esq, nome, path + " ESQ");
        boolean esqNao = esq.charAt(esq.length()-3) == 'N' && esq.charAt(esq.length()-2) == 'A' && esq.charAt(esq.length()-1) == 'O';
        if (!esqNao) return esq;
        String pathDir = Texto.faixa(esq, 0, esq.length() - 4);
        return pesquisarRec(n.dir, nome, pathDir + " DIR");
    }

    public void emOrdem() { emOrdem(raiz); }
    private void emOrdem(NoBST n) { if (n != null) { emOrdem(n.esq); n.avl.emOrdem(); emOrdem(n.dir); } }
}

public class Questao06tp04 {
    public static void main(String[] args) throws Exception {
        ColecaoRestaurantes colecao = ColecaoRestaurantes.lerCsv();
        Restaurante[] todos = colecao.getRestaurantes();
        ArvoreDupla estrutura = new ArvoreDupla();
        Scanner sc = new Scanner(System.in);
        int id = sc.nextInt();
        while (id != -1) {
            int i = 0; boolean enc = false;
            while (i < colecao.getTamanho() && !enc) { if (todos[i].getId() == id) { estrutura.inserir(todos[i]); enc = true; } i++; }
            id = sc.nextInt();
        }
        ...
        while (sc.hasNextLine()) {
            ...
            if (!Texto.igual(linha, "FIM")) {
                Restaurante r = colecao.buscarPorNome(linha);
                if (r != null) { estrutura.pesquisar(linha, r.getCapacidade()); }
                else { System.out.println("RAIZ NAO"); }
            }
        }
        long fim = System.currentTimeMillis();
        sc.close();
        long total = ArvoreDupla.comparacoesBST + AVL.comparacoes;
        java.io.FileWriter fw = new java.io.FileWriter("884985_hibrida_arvore_arvore.txt");
        fw.write("884985\t" + total + "\t" + String.format(Locale.US, "%.2f", (fim - inicio) / 1000.0) + "\n");
        fw.close();
    }
}
