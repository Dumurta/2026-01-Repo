...
class HashRehash {
    private static final int TAM = 83;
    private Restaurante[] tabela;
    static long comparacoes = 0;

    public HashRehash() { tabela = new Restaurante[TAM]; }

    private int asciiNome(String nome) { int soma = 0, i = 0; while (i < nome.length()) { soma += (int) nome.charAt(i); i++; } return soma; }
    private int h1(String nome) { return asciiNome(nome) % TAM; }
    private int h2(String nome) { return (asciiNome(nome) + 1) % TAM; }
    ...
    public void pesquisar(String nome) {
        int pos1 = h1(nome);
        comparacoes++;
        if (tabela[pos1] != null && Texto.igual(tabela[pos1].getNome(), nome)) {
            System.out.println(pos1 + " " + tabela[pos1].formatar()); return;
        }
        int pos2 = h2(nome);
        comparacoes++;
        if (tabela[pos2] != null && Texto.igual(tabela[pos2].getNome(), nome)) {
...
public class Questao04tp04 {
    public static void main(String[] args) throws Exception {
        ...
        while (id != -1) {
            ...
            while (i < colecao.getTamanho() && !enc) { if (todos[i].getId() == id) { hash.inserir(todos[i]); enc = true; } i++; }
            id = sc.nextInt();
        }
        ...
        while (sc.hasNextLine()) {
            linha = sc.nextLine();
            if (!Texto.igual(linha, "FIM")) { hash.pesquisar(linha); }
        }
        long fim = System.currentTimeMillis();
        sc.close();
        java.io.FileWriter fw = new java.io.FileWriter("884985_hash_rehash.txt");
        fw.write("884985\t" + HashRehash.comparacoes + "\t" + String.format(Locale.US, "%.2f", (fim - inicio) / 1000.0) + "\n");
        fw.close();
    }
}
