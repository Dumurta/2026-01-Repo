...
typedef struct { Restaurante* v[MAX_REGISTROS]; int topo; } Pilha;
void init_pilha(Pilha* p) { p->topo = 0; }
void empilhar(Pilha* p, Restaurante* r) { p->v[p->topo] = r; p->topo++; }
Restaurante* desempilhar(Pilha* p) { p->topo--; return p->v[p->topo]; }

int main() {
    ColecaoRestaurantes* c = ler_csv();
    Pilha p; init_pilha(&p);
    int id = 0; bool fim = false;
    while (scanf("%d", &id) == 1 && !fim) { if (id == -1) { fim = true; } else { empilhar(&p, buscar_por_id(c, id)); } }

    int ops = 0; scanf("%d", &ops);
    int i = 0;
    while (i < ops) {
