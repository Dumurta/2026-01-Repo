...
ColecaoRestaurantes *ler_csv()
{
    ...
    {
        fclose(a);
        ler_csv_colecao(c, (char *)PATH_CSV_LINUX);
    }
    else
    {
        ler_csv_colecao(c, (char *)PATH_CSV_WINDOWS);
    }
    return c;
}
...
typedef struct
{
    Restaurante *v[MAX_REGISTROS];
    int topo;
} Pilha;
void init_pilha(Pilha *p) { p->topo = 0; }
...
int main()
{
    clock_t inicio = clock() : ColecaoRestaurantes *c = ler_csv();
    Pilha p;
    init_pilha(&p);
    int id = 0;
    bool fim = false;
    while (scanf("%d", &id) == 1 && !fim)
    {
        if (id == -1)
        {
            fim = true;
    ...
    FILE *log = fopen("884985_questao12.txt", "w");
    fprintf(log, "884985\t%d\t%d\t%f", comparacoes, movimentacoes, tempo);
    fclose(log);
    return 0;
}
