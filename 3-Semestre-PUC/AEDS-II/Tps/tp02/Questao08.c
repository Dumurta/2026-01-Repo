...
int cmp_data(Data* a, Data* b) { int c = a->ano - b->ano; if (c == 0) { c = a->mes - b->mes; } if (c == 0) { c = a->dia - b->dia; } return c; }
...
void ler_csv_colecao(ColecaoRestaurantes* c, char* p) {
    ...
    char l[MAX_LINHA_CSV]; FILE* a = fopen(p, "r"); int i = 0;
    if (a != NULL) { if (fgets(l, sizeof(l), a) != NULL) {} while (fgets(l, sizeof(l), a) != NULL && i < c->tamanho) { int j = 0; while (l[j] != '\0') { if (l[j] == '\n') { l[j] = '\0'; } j++; } if (l[0] != '\0') { c->restaurantes[i] = parse_restaurante(l); i++; } } fclose(a); }
}
ColecaoRestaurantes* ler_csv() {
    ColecaoRestaurantes* c = (ColecaoRestaurantes*)malloc(sizeof(ColecaoRestaurantes)); FILE* a = fopen(PATH_CSV_LINUX, "r");
...
void quicksort(Restaurante** v, int esq, int dir) {
    int i = esq, j = dir;
    Restaurante* p = v[(esq + dir) / 2];
    while (i <= j) {
        while (cmp_data(&v[i]->data_abertura, &p->data_abertura) < 0) { i++; }
        while (cmp_data(&v[j]->data_abertura, &p->data_abertura) > 0) { j--; }
        if (i <= j) { Restaurante* t = v[i]; v[i] = v[j]; v[j] = t; i++; j--; }
    }
