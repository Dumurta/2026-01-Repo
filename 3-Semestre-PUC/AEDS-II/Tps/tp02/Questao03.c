...
#define PATH_CSV_WINDOWS "C:/tmp/restaurantes.csv" //fiz este TP no windows, entao coloquei um path pra funcionar no meu computador ao testar o programa, sei que nao e' o ideal mas nao tive acesso ao meu notebook com linux.
...
int texto_para_int(char* s) { int v = 0; sscanf(s, "%d", &v); return v; }
double texto_para_double(char* s) { double v = 0.0; sscanf(s, "%lf", &v); return v; }
...
void formatar_restaurante(Restaurante* r, char* b) {
    ...
    formatar_tipos(r, t); faixa_para_txt(r->faixa_preco, f); formatar_hora(&r->horario_abertura, ha); formatar_hora(&r->horario_fechamento, hf); formatar_data(&r->data_abertura, d);
...
int contar_registros(char* p) {
    int n = 0; char l[MAX_LINHA_CSV]; FILE* arq = fopen(p, "r");
...
void ler_csv_colecao(ColecaoRestaurantes* c, char* p) {
    ...
    char l[MAX_LINHA_CSV]; FILE* arq = fopen(p, "r"); int i = 0;
    if (arq != NULL) {
...
ColecaoRestaurantes* ler_csv() {
    ...
    if (a != NULL) { fclose(a); ler_csv_colecao(c, (char*)PATH_CSV_LINUX); } else { ler_csv_colecao(c, (char*)PATH_CSV_WINDOWS); }
    return c;
}
...
void liberar_colecao(ColecaoRestaurantes* c) {
    int i = 0; while (i < c->tamanho) { free(c->restaurantes[i]); i++; }
    free(c->restaurantes); free(c);
}

void selection_sort_nome(Restaurante** v, int n) {
    int i = 0;
    while (i < n - 1) {
        int m = i; int j = i + 1;
        while (j < n) { if (strcmp(v[j]->nome, v[m]->nome) < 0) { m = j; } j++; }
        if (m != i) { Restaurante* t = v[i]; v[i] = v[m]; v[m] = t; }
        i++;
    }
}
