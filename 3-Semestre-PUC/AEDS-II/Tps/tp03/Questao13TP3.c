...
NoArvore *novo_no(Restaurante *r)
{
...
static NoArvore *inserir_aux(NoArvore *no, Restaurante *r)
{
    NoArvore *resp = no;
    if (no == NULL)
    {
    ...
    {
        int cmp = strcmp(r->nome, no->restaurante->nome);
        if (cmp < 0)
        {
            no->esq = inserir_aux(no->esq, r);
        }
        else if (cmp > 0)
        {
            no->dir = inserir_aux(no->dir, r);
        }
    }
    return resp;
}

void inserir_restaurante(NoArvore **raiz, Restaurante *r)
{
    *raiz = inserir_aux(*raiz, r);
}

static void pesquisar_aux(NoArvore *no, char *nome, char *caminho)
{
    if (no == NULL)
    {
        printf("%s NAO\n", caminho);
    ...
    else
    {
        ...
        {
            char novo_caminho[MAX_LINHA_CSV];
            sprintf(novo_caminho, "%s esq", caminho);
            pesquisar_aux(no->esq, nome, novo_caminho);
        }
        else
        {
            char novo_caminho[MAX_LINHA_CSV];
            sprintf(novo_caminho, "%s dir", caminho);
            pesquisar_aux(no->dir, nome, novo_caminho);
        }
    }
}

void pesquisar_nome(NoArvore *raiz, char *nome)
{
    pesquisar_aux(raiz, nome, "raiz");
}

static void em_ordem_aux(NoArvore *no)
{
    if (no != NULL)
    {
        em_ordem_aux(no->esq);
...
int main()
{
    ...
    while (fgets(linha, sizeof(linha), stdin) != NULL)
    {
        ...
        {
            pesquisar_nome(raiz, linha);
        }
    }
    clock_t fim = clock();
    double tempo = ((double)(fim - inicio)) / CLOCKS_PER_SEC;
    em_ordem(raiz);
    FILE *log = fopen("884985_arvore_binaria.txt", "w");
