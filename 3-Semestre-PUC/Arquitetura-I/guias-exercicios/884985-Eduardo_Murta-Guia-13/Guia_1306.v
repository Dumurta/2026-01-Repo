// Guia_1306 - Contador em Anel 6 bits com JK
...
module q06_contador_anel_6bits (
  ...
  jkff ff0(.q(q0), .qnot(qn0), .j(q5),  .k(qn5), .clk(clk), .preset(1'b0), .clear(clear));
  ...
  jkff ff5(.q(q5), .qnot(qn5), .j(q4),  .k(qn4), .clk(clk), .preset(clear), .clear(1'b0));
...
module q06_test;
  ...
  q06_contador_anel_6bits DUT(.clk(clk), .clear(clear), .out(out));

  initial begin clk = 1'b0; forever #5 clk = ~clk; end

  initial begin
    $display("Guia_1306 - Eduardo Murta De Abreu - 884985");
    $display("Questao 06 - Contador em Anel 6 bits com JK");
    $display("-------------------------------------------");
