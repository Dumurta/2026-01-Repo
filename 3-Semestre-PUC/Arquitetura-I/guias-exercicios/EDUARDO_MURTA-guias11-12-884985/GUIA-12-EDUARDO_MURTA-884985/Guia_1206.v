// Guia_1206  - RAM 4x16 a partir de RAM 4x8 (JK)
...
module q06_ram4x16_from_4x8 (
  ...
  // baixo e alto (8 bits + 8 bits), mesmo addr/rw/clr/clk
...
module test;
  ...
  q06_ram4x16_from_4x8 DUT(clk, rw, addr, clr, in, out);
  ...
  // task de impressão
  task print_row;
    $display("| %3t |  %b  |  %2b  | %b  |  %b  | %016b | %016b |",
             $time, clk, addr, rw, clr, in, out);
  endtask

  initial begin
    // Cabeçalho
    $display("Guia_1206 - Eduardo Murta De Abreu - 884985");
    $display("Questao 6 - RAM 4x16 a partir de RAM 4x8 (JK)");
    $display("---------------------------------------------------------------------");
    $display("|  t  | clk | addr | rw | clr |      in (16)     |     out (16)     |");
    $display("---------------------------------------------------------------------");

    // reset
    clr=1; addr=2'b00; rw=0; in=16'h0000; #3; print_row();
    clr=0;                                #2; print_row();

    addr=2'b00; rw=1; in=16'hA1B2; @(posedge clk); print_row(); 
    rw=0;               @(negedge clk);           print_row();  

    addr=2'b10; rw=1; in=16'hC3D4; @(posedge clk); print_row(); 
    rw=0;               @(negedge clk);           print_row();  

    addr=2'b11; rw=1; in=16'h3C7E; @(posedge clk); print_row(); 
    rw=0;               @(negedge clk);           print_row();  
...
/*TESTE
Guia_1206 - Eduardo Murta De Abreu - 884985
Questao 6 - RAM 4x16 a partir de RAM 4x8 (JK) 
---------------------------------------------------------------------
|  t  | clk | addr | rw | clr |      in (16)     |     out (16)     |
---------------------------------------------------------------------
|   3 |  0  |  00  | 0  |  1  | 0000000000000000 | 0000000000000000 |
|   5 |  1  |  00  | 0  |  0  | 0000000000000000 | 0000000000000000 |
|  15 |  1  |  00  | 1  |  0  | 1010000110110010 | 0000000000000000 |
|  20 |  0  |  00  | 0  |  0  | 1010000110110010 | 1010000110110010 |
|  25 |  1  |  10  | 1  |  0  | 1100001111010100 | 0000000000000000 |
|  30 |  0  |  10  | 0  |  0  | 1100001111010100 | 1100001111010100 |
|  35 |  1  |  11  | 1  |  0  | 0011110001111110 | 0000000000000000 |
|  40 |  0  |  11  | 0  |  0  | 0011110001111110 | 0011110001111110 |
|  50 |  0  |  00  | 0  |  0  | 0011110001111110 | 1010000110110010 |
---------------------------------------------------------------------
*/