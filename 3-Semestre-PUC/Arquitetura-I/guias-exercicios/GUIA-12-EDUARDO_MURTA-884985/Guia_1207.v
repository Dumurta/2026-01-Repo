// -------------------------
// Guia_1207  - RAM 8x16 a partir de RAM 4x16 (JK)
...
module q04_ram4x8_from_2x8 (
  ...
  wire rw_g0 = rw & (addr[1] == 1'b0);
  wire rw_g1 = rw & (addr[1] == 1'b1);
...
module q05_ram8x8_from_4x8 (
  ...
  wire rw_g0 = rw & (addr[2] == 1'b0);
  wire rw_g1 = rw & (addr[2] == 1'b1);
...
// Q6) RAM 4x16 a partir de duas RAM 4x8 (Q4) em paralelo
module q06_ram4x16_from_4x8 (
  input  clk, input rw, input [1:0] addr, input clr,
  input  [15:0] in,
  output [15:0] out
);
  wire [7:0] lo_out, hi_out;

  q04_ram4x8_from_2x8 LO(clk, rw, addr, clr, in[7:0],   lo_out);
  q04_ram4x8_from_2x8 HI(clk, rw, addr, clr, in[15:8],  hi_out);
...
// ------------------------------------------------------------
// Q7) RAM 8x16 a partir de duas RAM 4x16 (Q6)
// ------------------------------------------------------------
module q07_ram8x16_from_4x16 (
  input  clk, input rw, input [2:0] addr, input clr,
  input  [15:0] in,
  output [15:0] out
);
  ...
  // saídas de cada grupo 4x16
  wire [15:0] g0_out, g1_out;

  // dentro de cada grupo, o endereço local é addr[1:0]
  q06_ram4x16_from_4x8 G0(clk, rw_g0, addr[1:0], clr, in, g0_out);
  q06_ram4x16_from_4x8 G1(clk, rw_g1, addr[1:0], clr, in, g1_out);
...
module test;
  ...
  q07_ram8x16_from_4x16 DUT(clk, rw, addr, clr, in, out);
  ...
  // task de impressão
  task print_row;
    $display("| %3t |  %b  |  %3b | %b  |  %b  | %016b | %016b |",
             $time, clk, addr, rw, clr, in, out);
  endtask

  initial begin
    // Cabeçalho
    $display("Guia_1207 - Eduardo Murta De Abreu - 884985");
    $display("Questao 7 - RAM 8x16 a partir de RAM 4x16 (JK)");
    $display("---------------------------------------------------------------------");
    $display("|  t  | clk | addr | rw | clr |      in (16)     |     out (16)     |");
    $display("---------------------------------------------------------------------");

    // reset
    clr=1; addr=3'b000; rw=0; in=16'h0000; #3; print_row();
    clr=0;                              #2; print_row();

    addr=3'b000; rw=1; in=16'hABCD; @(posedge clk); print_row(); 
    rw=0;              @(negedge clk);           print_row();    

    addr=3'b011; rw=1; in=16'h1234; @(posedge clk); print_row(); 
    rw=0;              @(negedge clk);           print_row();  

    addr=3'b100; rw=1; in=16'hDEAD; @(posedge clk); print_row(); 
    rw=0;              @(negedge clk);           print_row();     

    addr=3'b111; rw=1; in=16'hBEEF; @(posedge clk); print_row(); 
    rw=0;              @(negedge clk);           print_row();     
...
/*TESTE
Guia_1207 - Eduardo Murta De Abreu - 884985
Questao 7 - RAM 8x16 a partir de RAM 4x16 (JK)
---------------------------------------------------------------------
|  t  | clk | addr | rw | clr |      in (16)     |     out (16)     |
---------------------------------------------------------------------
|   3 |  0  |  000 | 0  |  1  | 0000000000000000 | 0000000000000000 |
|   5 |  1  |  000 | 0  |  0  | 0000000000000000 | 0000000000000000 |
|  15 |  1  |  000 | 1  |  0  | 1010101111001101 | 0000000000000000 |
|  20 |  0  |  000 | 0  |  0  | 1010101111001101 | 1010101111001101 |
|  25 |  1  |  011 | 1  |  0  | 0001001000110100 | 0000000000000000 |
|  30 |  0  |  011 | 0  |  0  | 0001001000110100 | 0001001000110100 |
|  35 |  1  |  100 | 1  |  0  | 1101111010101101 | 0000000000000000 |
|  40 |  0  |  100 | 0  |  0  | 1101111010101101 | 1101111010101101 |
|  45 |  1  |  111 | 1  |  0  | 1011111011101111 | 0000000000000000 |
|  50 |  0  |  111 | 0  |  0  | 1011111011101111 | 1011111011101111 |
|  60 |  0  |  000 | 0  |  0  | 1011111011101111 | 1010101111001101 |
|  70 |  0  |  011 | 0  |  0  | 1011111011101111 | 0001001000110100 |
---------------------------------------------------------------------
*/